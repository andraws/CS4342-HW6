...
def findBestHyperParameters(X, y, w1, w2, b1, b2, faces, labels):
    ...
    epochs = [20, 30, 40, 50]
    regStrength = [0.05, 0.1, 0.5, 1, 5]

    for i in range(10):
        print("Validation {}".format(i))
        currentHidden = hiddenLayers[int(np.random.rand()*3)]
        currentLearning = learningRate[int(np.random.rand()*6)]
        currentBatch = batchSize[int(np.random.rand()*5)]
        currentEpochs = epochs[int(np.random.rand()*4)]
        
        print("Hidden layers = ", currentHidden)
        print("Learning Rate = ", currentLearning)
        print("Batch Size = ", currentBatch)
        print("Epochs = ", currentEpochs)
